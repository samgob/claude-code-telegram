...
- **`/use` accepts natural-language queries** in addition to id prefixes. `/use latest wesco` runs keyword search across recent transcripts, tokenized with stopword + recency-hint stripping (`the`, `session`, `latest`, etc.). Single match → resumes immediately. Multiple matches → short disambiguation list. When the query carries a recency hint (`latest`, `most recent`, `newest`), results are sorted by mtime, with a relevance floor that drops 1-hit incidental mentions if any multi-hit matches exist (prevents routine sessions that mention a keyword in passing from outranking sessions focused on it).
