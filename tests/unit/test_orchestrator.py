...
def test_agentic_registers_9_commands(agentic_settings, deps):
    """Agentic mode registers start, help, new, status, verbose, repo,
    sessions, use, restart commands."""
    ...
    assert frozenset({"use"}) in commands
...
def test_agentic_registers_text_document_photo_handlers(agentic_settings, deps):
    ...
    # 4 callback handlers (stop: + cd: + use_more + use:)
